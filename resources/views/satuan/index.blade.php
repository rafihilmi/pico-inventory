...
<div class="container mx-auto max-w-6xl" x-data="{ showModal: false, showEditModal: false, editData: {} }">
    <div class="text-[#3c8dbc] text-sm font-semibold mb-6 flex items-center gap-2">
        <span>Dashboard</span> <span class="text-gray-400">/</span> <span>Satuan</span> <span class="text-gray-400">/</span> <span class="text-gray-500">Data Satuan</span>
    </div>
    ...
    <div class="bg-white border border-gray-300 btn-sq mb-6">
        <div class="border-b border-gray-200 bg-white p-3 flex items-center gap-2 text-gray-700 font-bold text-lg">
            ...
            Data satuan <span class="bg-gray-400 text-white text-xs px-2 py-0.5 rounded-sm ml-1">{{ $satuans->count() }}</span>
        </div>
        ...
        <div class="p-4">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('satuan.index') }}" class="bg-[#f4f4f4] border border-[#ddd] hover:bg-[#e7e7e7] text-gray-700 px-3 py-1.5 btn-sq text-sm flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                        Refresh
                    </a>
                </div>
                <form action="{{ route('satuan.index') }}" method="GET" class="flex gap-2 w-full sm:w-auto">
                    <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari satuan..." class="flex-1 sm:w-64 border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:border-[#3c8dbc]">
                        ...
                        <tr class="hover:bg-gray-50 border-b border-gray-100">
                            <td class="px-2 py-2">
                                <div class="flex gap-1">
                                    <button @click="showEditModal = true; editData = { id: '{{ $s->id_satuan }}', kode: '{{ addslashes($s->kode) }}', nama: '{{ addslashes($s->nama) }}' }" class="bg-[#00a65a] text-white p-1 btn-sq hover:bg-[#008d4c]" title="Edit">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    <form action="{{ route('satuan.destroy', $s->id_satuan) }}" method="POST" onsubmit="return confirm('Hapus satuan ini?')">
                                        @csrf
                                        @method('DELETE')
                            ...
                            <td class="px-3 py-2 text-gray-800">{{ $s->kode }}</td>
                            <td class="px-3 py-2 text-gray-800">{{ $s->nama }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="4" class="px-3 py-4 text-center text-gray-400">Belum ada data satuan.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" x-cloak style="display: none;">
        <div class="w-full max-w-2xl bg-white btn-sq shadow-xl" @click.away="showModal = false">
            <div class="border-b border-gray-200 p-4 flex justify-between items-center bg-white">
                <h2 class="text-lg text-gray-700">Tambah Satuan</h2>
            ...
            <form action="{{ route('satuan.store') }}" method="POST">
                ...
                <div class="p-6 grid grid-cols-1 gap-4">
                    <div class="flex items-center">
                        <label class="w-32 text-sm text-gray-600 shrink-0">Kode</label>
                    ...
                    <div class="flex items-center">
                        <label class="w-32 text-sm text-gray-600 shrink-0">Nama Satuan</label>
                        <span class="mr-2 text-gray-600">:</span>
    ...
    <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" x-cloak style="display: none;">
        <div class="w-full max-w-2xl bg-white btn-sq shadow-xl" @click.away="showEditModal = false">
            <div class="border-b border-gray-200 p-4 flex justify-between items-center bg-white">
                <h2 class="text-lg text-gray-700">Edit Satuan</h2>
            ...
            <form :action="'{{ route('satuan.index') }}/' + editData.id" method="POST">
                @csrf
                @method('PUT')
                <div class="p-6 grid grid-cols-1 gap-4">
                    <div class="flex items-center">
